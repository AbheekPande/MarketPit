...
def api_options():
    """
    NSE Options Chain — fetches live options data from NSE India.
    Uses session warmup to bypass NSE bot protection.
    Returns: {symbol, spot, atm, expiry, expiryDates, rows:[{strike, CE:{ltp,oi,oiChg,iv,vol,bid,ask,chg}, PE:{...}}], pcr}
    """
    if req_lib is None:
        return jsonify({"error": "requests library not available"}), 503

    symbol   = request.args.get("symbol", "NIFTY").upper()
    nse_syms = {"NIFTY": "NIFTY", "BANKNIFTY": "BANKNIFTY", "FINNIFTY": "FINNIFTY", "MIDCPNIFTY": "MIDCPNIFTY"}
    nse_sym  = nse_syms.get(symbol, "NIFTY")
    ...
    try:
        # Warm up session (NSE requires cookies)
        session = req_lib.Session()
        session.get("https://www.nseindia.com", headers=HEADERS, timeout=12, verify=False)
        time.sleep(1.2)
        session.get("https://www.nseindia.com/option-chain", headers=HEADERS, timeout=10, verify=False)
        time.sleep(0.8)

        url  = f"https://www.nseindia.com/api/option-chain-indices?symbol={nse_sym}"
        resp = session.get(url, headers=HEADERS, timeout=18, verify=False)
        if resp.status_code != 200:
            return jsonify({"error": f"NSE returned {resp.status_code}"}), 502
        ...
        # Filter to first expiry
        data = [r for r in records.get("data", []) if r.get("expiryDate") == expiry]

        # Build rows
        strikes = sorted(set(r["strikePrice"] for r in data if "strikePrice" in r))
        atm     = min(strikes, key=lambda s: abs(s - spot), default=spot)
        ...
        for r in data:
            ...
            if s not in strike_map:
                strike_map[s] = {"CE": {}, "PE": {}}
            side = "CE" if "CE" in r else "PE" if "PE" in r else None
            if side:
                d2 = r[side]
                strike_map[s][side] = {
                    "ltp":    d2.get("lastPrice", 0),
                    "oi":     d2.get("openInterest", 0),
                    "oiChg":  d2.get("changeinOpenInterest", 0),
                    "vol":    d2.get("totalTradedVolume", 0),
                    "iv":     d2.get("impliedVolatility", 0),
                    "bid":    d2.get("bidprice", 0),
                    "ask":    d2.get("askPrice", 0),
                    "chg":    d2.get("pChange", 0),
                }

        for strike in sorted(strike_map):
            rows.append({"strike": strike, "CE": strike_map[strike]["CE"], "PE": strike_map[strike]["PE"]})
